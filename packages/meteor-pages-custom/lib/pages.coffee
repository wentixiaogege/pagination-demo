 ...
 # Sets the options for this instance
  ...
  setTemplates: ->
    ...
    for i in [@navTemplate, @pageTemplate, @itemTemplate, @tableTemplate]
      tn = @id + i #"pages_items_pagesNavCont" because id is "pages_items"
      Template[tn] = new Blaze.Template "Template.#{tn}", Template[i].renderFunction
      Template[tn].helpers _TemplateHelpers[i] # _TemplateHelpers is a dictionary with keys equal to the names of the templates in the array above, each having its specifc helpers
      # _pagesNav, for example, has link (a function), navigationNeighbors (a function), and show (a function)
      # show returns true if this collection has more than one page
      Template[tn].events _TemplateEvents[i]
      # only _pagesNav has events, and only one event: 'click a': 
      ###
      Here it is: 
    ...
    Template[name].helpers # name equals "Items" here, so I guess this adds these templates as helpers so they can be called there, idk
      pagesData: @
